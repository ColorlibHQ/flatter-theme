...
function flatter_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'flatter' ),
        'next_string'     => esc_html__( '&raquo;', 'flatter' ),
        'before_output'   => '<div class="flatter-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'flatter_pagination_defaults', $defaults )
    );
