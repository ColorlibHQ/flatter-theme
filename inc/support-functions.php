...
function flatter_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'flatter' ) . '</span></li>';
...
// Blog categories
function flatter_featured_post_cat(){
...
// social media
if ( ! function_exists( 'flatter_social' ) ) {
	function flatter_social( $args = array() ) {
		...
		$url = flatter_opt( 'flatter_social_url' );
...
// header cart count
function flatter_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( FLATTER_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'flatter' ); ?>">
...
function flatter_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'flatter_contact7_form_content', 10, 2 );